...
export async function getComments(tid: string, parent?: string) {
    const response = await axiosInst.get(`forum/comment-view/?thread=${tid}&?parent=${parent}`)

    return response
}

export async function createComment(tid: string, parent?: string) {
    const response = await axiosInst.post(`forum/comment-view/?thread=${tid}&?parent=${parent}`)
